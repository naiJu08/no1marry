...
class ChatController extends Controller
{
    ...
    public function create_chat(Request $request){
        ...
        $existing_chat_thread = ChatThread::where('sender_user_id', Auth::user()->id)->where('receiver_user_id', $request->id)->first();
        $existing_chat_thread2 = ChatThread::where('sender_user_id', $request->id)->where('receiver_user_id',Auth::user()->id )->first();
          if ($existing_chat_thread == null &&$existing_chat_thread2==null){
              $chat_thread                    = new ChatThread;
              $chat_thread->thread_code       = Auth::user()->id.date('Ymd').$request->id;
              $chat_thread->sender_user_id    = Auth::user()->id;
              $chat_thread->receiver_user_id  = $request->id;
              $chat_thread->save();
          }
          return 1;
    }
