...
<div class="glass-header position-sticky w-100 top-0 z-1020 bg-white shadow-sm">
    <div class="glass-header__top py-1 bg-white border-bottom">
        <div class="container-fluid d-flex align-items-center justify-content-between">
            <a href="{{ get_setting('header_left_quick_link1') }}"
                class="glass-link d-none d-md-inline-flex align-items-center">
    ...
    <header class="glass-header__main">
        <div class="container-fluid">
            <div class="glass-header__bar d-flex align-items-center">
                ...
                <div class="glass-header__nav collapse d-lg-flex flex-grow-1 justify-content-center order-2"
                    id="primaryNav">
                    <nav class="glass-nav">
                ...
                <div class="glass-header__bar-actions d-flex align-items-center ml-auto order-3">
                    ...
                    @if (Auth::check())
                        ...
                        <a href="{{ route('logout') }}"
                            class="btn btn-sm btn-outline-primary rounded-pill shadow-sm d-none d-md-inline-flex">{{ translate('Logout') }}</a>
                    @else
                        <a class="btn btn-sm btn-outline-primary rounded-pill shadow-sm d-none d-md-inline-flex mr-2"
                            href="{{ route('login') }}">{{ translate('Log In') }}</a>
                        <a class="btn btn-sm btn-primary rounded-pill shadow-sm d-none d-md-inline-flex"
                            href="{{ route('register') }}">{{ translate('Create Profile') }}</a>
                    @endif
...
<div id="notifSidebar" class="notif-sidebar">
    <div class="notif-sidebar__header">
        ...
        <button type="button" id="notifSidebarClose"
            class="btn btn-sm btn-outline-secondary rounded-pill">&times;</button>
    </div>
...
<div id="chatSidebar" class="notif-sidebar">
    <div class="notif-sidebar__header">
        ...
        <button type="button" id="chatSidebarClose"
            class="btn btn-sm btn-outline-secondary rounded-pill">&times;</button>
    </div>
    <div class="notif-sidebar__body c-scrollbar-light">
                ...
                <a href="{{ route('all.messages') }}" class="chat-user-item p-3 d-block text-inherit hov-bg-soft-primary">
                    <div class="media">
                        ...
                        <div class="ml-2 text-right">
                            <div class="opacity-60 fs-10 mb-1">{{ Carbon\Carbon::parse($chat->created_at)->diffForHumans() }}
                            </div>
                        </div>
                    </div>
                </a>
...
<div class="d-lg-none">
    ...
    <div id="mobileSidebar" class="mobile-sidebar"
            ...
            <div class="d-flex align-items-center">
                ...
                <div class="d-flex flex-column">
                    <small class="text-uppercase text-muted"
                        style="font-size:.68rem;letter-spacing:.12em;">{{ translate('Quick menu') }}</small>
                    @if($mobileUser)
        ...
        <div class="ms-drawer-body">
            ...
            @if(Auth::check())
                ...
                <a href="javascript:void(0)" onclick="confirmDeleteAccount()" class="ms-link mt-2"
                    style="background: rgba(239, 68, 68, 0.05); border: 1px solid rgba(239, 68, 68, 0.15);">
                    <i class="las la-trash-alt mr-2" style="color: #ef4444;"></i>
                    <span style="color: #ef4444; font-weight: 600;">{{ translate('Delete Account') }}</span>
                </a>
                <form id="delete-account-form-sidebar" action="{{ route('member.delete_account') }}" method="POST"
                    style="display: none;">
                    @csrf
                </form>
            @else
    ...
    <nav class="mobile-bottom-nav d-flex justify-content-around align-items-center px-3"
        style="position:fixed;bottom:0;left:0;right:0;height:72px;z-index:1051;padding-bottom:env(safe-area-inset-bottom);">
        ...
        <a href="{{ route('member.listing') }}" class="mb-item {{ areActiveRoutes(['member.listing'], 'active') }}"
        ...
        <a href="{{ route('packages') }}" class="mb-item {{ areActiveRoutes(['packages'], 'active') }}"
            aria-label="{{ translate('Plans') }}">
            <span class="mb-item-icon"><i class="las la-gem"></i></span>
            <span class="mb-item-label">{{ translate('Plans') }}</span>
        </a>
        <a href="{{ url('/help_and_support') }}" class="mb-item {{ request()->is('help_and_support') ? 'active' : '' }}"
        ...
        @if(Auth::check())
            <a href="{{ route('all.messages') }}" class="mb-item {{ areActiveRoutes(['all.messages'], 'active') }}"
            ...
            <a href="{{ route('dashboard') }}" class="mb-item {{ areActiveRoutes(['dashboard'], 'active') }}"
    ...
    <style>
        ...
        .mobile-sidebar .ms-link:hover {
            background: rgba(255, 255, 255, 0.96);
            box-shadow: 0 14px 32px rgba(15, 23, 42, .24);
        }

        .mobile-sidebar .ms-link:active {
            transform: scale(.98)
        }

        .mobile-sidebar .ms-link i {
            font-size: 20px;
            width: 24px;
            text-align: center;
            margin-right: 8px
        ...
        .mobile-bottom-nav {
            background: radial-gradient(circle at 0% 0%, rgb(1 40 78 / 96%), transparent 60%), radial-gradient(circle at 120% -10%, rgba(191, 219, 254, 0.85), transparent 55%), rgba(15, 23, 42, 0.92);
        ...
        .mobile-bottom-nav .mb-item-icon {
            display: block;
            font-size: 20px;
            line-height: 1
        }

        .mobile-bottom-nav .mb-item-label {
            display: block;
            font-size: 10px;
            margin-top: 3px;
            font-weight: 500;
        ...
        .mobile-bottom-nav .mb-item:hover:not(.active) {
...
<script>
    ...
    function confirmDeleteAccount() {
        if (confirm("{{ translate('Are you sure you want to delete your account? Permanently remove all your data, photos, and messages. This action cannot be undone.') }}")) {
