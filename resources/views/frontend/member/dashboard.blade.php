...
@section('panel_content')
    ...
    <section id="member-dashboard">
            ...
            <div class="dashboard-container">
                <div class="hero-content">
                    <div class="hero-intro">
                        ...
                        <p>{{ __('Keep discovering meaningful matches curated especially for you. Fine-tune your profile, explore premium connections, and let your story unfold in style.') }}
                        </p>
                    ...
                    <div class="hero-media">
                        <div class="hero-avatar-wrapper"
                            style="margin-bottom: 1.5rem; display:flex; justify-content:flex-start;">
                            <div class="profile-avatar">
                                <div class="profile-avatar-badge">
                                    <div class="profile-avatar-backdrop"></div>
                                    <div class="profile-avatar-ring"></div>
                                    <img class="profile-avatar-image"
                                        src="@if(Auth::user()->photo != "") {{ uploaded_asset(Auth::user()->photo) }} @else {{ static_asset('assets/img/avatar-place.png') }} @endif"
                                        alt="Profile Image">
                                    <form action="{{ route('uplode.uplode_pro_pic') }}" method="POST" name="pro_form"
                                        id="pro_form" enctype="multipart/form-data">
                                        @csrf
                                        <button type="button" class="profile-avatar-trigger" id="upload-button-pro"
                                            data-upload-sheet="enabled">
                        ...
                        <p>{{ __('Profiles with complete details receive 3x more responses. Update your preferences and add recent photographs to stand out.') }}
                        </p>
                        ...
                        <div class="hero-actions">
                            <a class="primary-action"
                                href="{{ Auth::check() ? route('profile_settings') : 'javascript:void(0);' }}">
        ...
        <div class="dashboard-container">
                ...
                <div class="match-grid">
                    @foreach ($newMatchCollection as $member)
                        ...
                        <div class="match-card">
                            ...
                            <img class="content-image @if($profile_picture_show != 'ok' || ($member->membership == 2 && Auth::check() && Auth::user()->membership == 1)) restricted-image @endif @if($should_blur) blur @endif"
                                @if($member->photo != "") src="{{ uploaded_asset($member->photo) }}" @else
                                src="{{ static_asset('assets/img/avatar-place.png') }}" @endif alt="{{ $member->first_name }}">
            ...
            @if($premiumMatchCollection->count())
                <div class="dashboard-section">
                    <div class="section-header">
                        ...
                        <a href="javascript:void(0);">{{ __('Upgrade to unlock more') }} <i
                    ...
                    <div class="premium-strip">
                        @foreach ($premiumMatchCollection as $member)
                            ...
                            <div class="match-card">
                                <div class="content-overlay1"></div>
                                <img class="content-image @if($profile_picture_show != 'ok' || (Auth::check() && Auth::user()->membership == 1)) restricted-image @endif @if($should_blur) blur @endif"
                                    @if($member->photo != "") src="{{ uploaded_asset($member->photo) }}" @else
                                    src="{{ static_asset('assets/img/avatar-place.png') }}" @endif alt="{{ $member->first_name }}">
                                <div class="crown">
                    ...
                    <div class="glass-panel">
                        ...
                        <div class="text-center py-5">
                            ...
                            <p class="mb-0 text-muted small">
                                {{ __('Keep exploring and connecting – your story could be the next one we celebrate here.') }}
                            </p>
                        </div>
                    </div>
                    <div class="glass-panel">
                        ...
                        <div class="text-center py-5">
                            ...
                            <p class="mb-0 text-muted small">
                                {{ __('Stay tuned for upcoming events and exclusive community updates right here.') }}</p>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="cta-banner">
                <div>
                    ...
                    <p>{{ __('Unlock priority matchmaking, personal relationship managers, and handpicked introductions tailored to your aspirations.') }}
                    </p>
                </div>
        ...
        <div class="dashboard-container delete-account-section" style="margin-top: 2rem; margin-bottom: 2rem;">
            <div class="glass-panel"
                style="background: linear-gradient(135deg, rgba(255, 245, 245, 0.95), rgba(255, 240, 240, 0.9)); border: 1px solid rgba(239, 68, 68, 0.2); box-shadow: 0 20px 45px rgba(239, 68, 68, 0.1); padding: 2rem;">
                <div class="row align-items-center">
                    <div class="col-md-8">
                        <div class="d-flex align-items-center">
                            <div
                                style="width: 48px; height: 48px; background: linear-gradient(135deg, #ef4444, #dc2626); border-radius: 12px; display: flex; align-items: center; justify-content: center; margin-right: 1.5rem; box-shadow: 0 8px 20px rgba(239, 68, 68, 0.25); flex-shrink: 0;">
                                <i class="fa-solid fa-trash-alt text-white" style="font-size: 1.2rem;"></i>
                            </div>
                            <div>
                                <h5 style="color: #dc2626; margin-bottom: 0.5rem; font-weight: 700; font-size: 1.1rem;">
                                    {{ __('Delete Your Account') }}</h5>
                    ...
                    <div class="col-md-4 text-md-end mt-3 mt-md-0">
                        <form action="{{ route('member.delete_account') }}" method="POST" id="deleteAccountForm">
                            @csrf
                            <button type="submit" class="btn btn-danger delete-account-btn"
                                style="padding: 0.75rem 2rem; font-weight: 600; border-radius: 12px; box-shadow: 0 8px 25px rgba(239, 68, 68, 0.3); transition: all 0.3s ease; border: none; background: linear-gradient(135deg, #ef4444, #dc2626); min-width: 100%;">
