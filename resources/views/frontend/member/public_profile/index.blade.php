...
<style>
    ...
    .pp-card {
        ...
        border-radius: 12px;
        background:
            radial-gradient(circle at 0% 0%, rgba(255,255,255,0.96), transparent 58%),
            radial-gradient(circle at 120% -10%, rgba(219,234,254,0.9), transparent 55%),
            #ffffff;
    ...
    .pp-card::before {
        ...
        inset: 0;
        pointer-events: none;
        background: linear-gradient(135deg, rgba(109,75,255,0.12), rgba(247,79,143,0.08));
        opacity: 0.35;
    ...
    .pp-upgrade-card {
        ...
        background:
            radial-gradient(circle at 0% 0%, rgba(255,255,255,0.82), transparent 58%),
            radial-gradient(circle at 100% 0%, rgba(254,249,195,0.7), transparent 55%),
            linear-gradient(135deg, #fce7f3, #fef3c7);
        box-shadow: 0 22px 40px rgba(15, 10, 43, 0.16);
        display: flex;
        align-items: center;
        justify-content: space-between;
        gap: 1.25rem;
    ...
    <div class="container pp-wrapper">
        ...
        @else
            <div class="pp-upgrade-card">
                <div>
                    <div class="pp-upgrade-pill">
                        <i class="la la-crown"></i>
                        <span>{{ translate('Premium Access') }}</span>
                    </div>
                    <div class="pp-upgrade-title">
                        {{ translate('Upgrade your package to unlock this full profile') }}
                    </div>
                    <p class="pp-upgrade-copy mb-0">
                        {{ translate('Choose a suitable package to view complete details, including contact number, verified information and full profile sections.') }}
                    </p>
                </div>
                <div class="pp-upgrade-actions">
                    <button type="button" class="pp-upgrade-btn" onclick="window.location='{{ route('packages') }}'">
                        <span>{{ translate('CHOOSE A PACKAGE') }}</span>
                        <i class="la la-arrow-right"></i>
                    </button>
                    <span class="pp-upgrade-note">
                        {{ translate('You can continue browsing profiles even before upgrading.') }}
                    </span>
                </div>
            </div>
        @endif
