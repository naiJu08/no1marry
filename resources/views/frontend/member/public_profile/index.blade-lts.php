...
.custom-button {
    background-color: #522546 !important; /* Primary blue */
    color: white;
    font-weight: bold;
    border: none;
    padding: 10px 20px;
...
.profile-section-card {
    ...
    box-shadow: 0 18px 40px rgba(24, 12, 58, 0.08);
}
...
.profile-section-card th,
.profile-section-card td {
    border: 0;
    padding: 0.35rem 0;
}
...
.profile-section-card th {
    font-weight: 600;
    color: rgba(32,21,56,0.82);
    width: 42%;
}

.profile-section-card td {
    color: rgba(32,21,56,0.86);
}
...
<section class="pt-6 bg-primary-grad text-white" style="background-color:rgb(163 13 13);">
    <div class="container public-profile-wrapper">
        <div class="row">
...
<section class="py-5 bg-white">
    ...
    @php
        $profile_picture_show = 'ok';
        $profile_picture_privacy = $user->member->profile_picture_privacy ?? null;
        ...
        $can_view_image = Auth::check() && (
            in_array(Auth::user()->membership, [3, 2]) || $interest_accepted || $is_female_user
        );

        $blur_class = ($profile_picture_show == 'no' && !$can_view_image) ? 'blur' : '';
    @endphp
    ...
    <img
        ...
        @else
            src="{{ static_asset('assets/img/avatar-place.png') }}"
        @endif
        onerror="this.onerror=null;this.src='{{ uploaded_asset($user->photo) }}';"
        class="img-fluid w-100 {{ $blur_class }}"
        alt="user">
</div>
...
<div class="overflow-hidden rounded shadow-lg mb-4 bg-white d-xl-none">
    <img
        src="{{ uploaded_asset($user->photo) }}"
        onerror="this.onerror=null;this.src='{{ static_asset('assets/img/avatar-place.png') }}';"
        class="img-fluid w-100 {{ $can_view_image ? '' : 'blur' }}"
        alt="no.1marry">
</div>
            ...
            <div class="modal-dialog">
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        <p>{{ translate('Please choose a package to view the full profile with contact number.') }}</p>
                    </div>
        ...
        <div class="row gutters-10">
            <div class="col-12 col-md-6">
                <label class="aiz-megabox d-block mb-3">
                    <a href="{{ route('packages') }}" class="text-primary font-weight-bold d-flex align-items-center">
                        
                      
                        <button class="btn btn-primary custom-button" onclick="window.location='{{ route('packages') }}'">
     <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" fill="currentColor" class="bi bi-box-seam mr-2" viewBox="0 0 16 16">
...
<div id="contact-details" class="ml-3 ml-md-5 pl-25px">
            ...
            @elseif ($current_package_id == 9)
                @if (empty($do_expressed_interest) && empty($received_expressed_interest))
                    <a onclick="express_interest({{ $user->id }})"
                       class="btn w-100 bg-primary text-white mt-2 view_contact">
                        <i class="las la-phone"></i> {{ translate('Can You please Express Your Interest') }}
                    </a>
                    <div>contact admin for direct profile assistance</div>
                @elseif (
                    (!empty($received_expressed_interest) && $received_expressed_interest->status == 1) ||
                ...
                @else
                    <div class="d-flex mb-3">
                        ...
                        <div>
                            <div class="fs-15 fw-600 mb-1">{{ translate('Contact Number') }}</div>
                            <div class="fw-400">wait until the Interest is accepted</div>
                        </div>
                    </div>
                @endif
            @endif
    ...
    <!--</div>-->
        <!--<div id="contact-details" class="ml-3 ml-md-5 pl-25px">-->
        <!--<div class="row">-->
        <!--    @if (Auth::check() && Auth::user()->membership == 4)-->
        <!--                            <a -->
        <!--                   class="btn w-100 bg-primary text-white mt-2 view_contact">-->
        <!--                    <i class="las la-phone"></i> {{ $user->phone }}-->
        <!--                </a>-->
        <!--                @else-->
        <!--                            <div class="col-md-6">-->
        <!--        <div class="border p-3">-->
        ...
        <!--                </a>-->
        <!--                <div>contact admin for direct profile assistance</div>-->
        <!--            @elseif(!empty($received_expressed_interest) && $received_expressed_interest->status == 1)-->
        <!--                <div class="d-flex mb-3">-->
        <!--                    <i class="las la-phone text-primary la-2x mr-3"></i>-->
        <!--                    <div>-->
        <!--                        <div class="fs-15 fw-600 mb-1">{{ translate('Contact Number') }}</div>-->
        ...
        <!--                    </div>-->
        <!--                </div>-->
        <!--            @elseif(!empty($do_expressed_interest) && $do_expressed_interest->status == 1)-->
        <!--                <div class="d-flex mb-3">-->
        <!--                    <i class="las la-phone text-primary la-2x mr-3"></i>-->
        <!--                    <div>-->
        <!--                        <div class="fs-15 fw-600 mb-1">{{ translate('Contact Number') }}</div>-->
...
<!--                <div id="contact-details" class="collapse accordion-body" data-parent="#profile-accordion">-->
<!--    <div class="row">-->
<!--        @if (Auth::check())-->
<!--            @if (Auth::user()->member_contacts > 0)-->
<!--                @php-->
<!--                    // Deduct 1 from member_contacts when viewing the contact number-->
<!--                    Auth::user()->decrement('member_contacts');-->
<!--                    Auth::user()->save();-->
<!--                @endphp-->
...
<!--                </div>-->
                <!--<div>-->
                <!--    You have {{ $user->member_contacts -1 }} contact views remaining-->
                <!--</div>-->
<!--            @else-->
<!--                <div class="d-flex mb-3">-->
<!--                    <i class="las la-phone text-primary la-2x mr-3"></i>-->
<!--                    <div>-->
<!--                        <div class="fs-15 fw-600 mb-1">{{ translate('Contact Number') }}</div>-->
<!--                        <div class="fw-400">{{ translate('You have no remaining contact views, contact admin to add more contact views') }}</div>-->
<!--                    </div>-->
<!--                </div>-->
<!--            @endif-->
<!--        @endif-->
...
<!--    </div>-->
                            ...
                            @if(get_setting('member_permanent_address_section') == 'on')
                                <!-- Permanent Address -->
                              @php $permanent_address = \App\Models\Address::where('user_id',$user->id)->where('type','present')->first(); @endphp
                                                ...
                                                <table class="w-100">
                                                    <tbody>
                                                        <tr>
                                                            ...
                                                            <td class="py-1">{{ !empty($permanent_address->city->name) ? $permanent_address->city->name : "N/A" }}</td>
                                                        </tr>
                                                        <tr>
                                                            <th class="py-1">{{translate('Country')}}</th>
                                                            <td class="py-1">{{ !empty($permanent_address->country->name) ? $permanent_address->country->name : "N/A" }}</td>
                                                        </tr>
                                                    </tbody>
                                                </table>
                                            </div>
                                                ...
                                                <table class="w-100 ml-sm-4">
                                                    <tbody>
                                                        <tr>
                                                            ...
                                                            <td class="py-1">{{!empty($permanent_address->state->name) ? $permanent_address->state->name : "N/A" }}</td>
                                                        </tr>
                                                        <tr>
                                                            <th class="py-1">{{translate('Postal Code')}}</th>
                                                            <td class="py-1">{{ !empty($permanent_address->postal_code) ? $permanent_address->postal_code : "N/A" }}</td>
                                                        </tr>
                                                    </tbody>
                                                </table>
                                            </div>
