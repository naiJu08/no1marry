        ...
        <div class="media">
            ...
            <span class="avatar avatar-xs flex-shrink-0">
                @if ($chat->sender->photo != null)
                    <img src="{{ uploaded_asset($chat->sender->photo) }}" alt="no.1marry">
                @else
                    <img src="{{ static_asset('assets/frontend/img/avatar-place.png') }}" alt="no.1marry">
                @endif
            </span>
        </div>
    </div>
                            ...
                            @if ($attachment->type == 'image')
                                <div class="mb-2 file-preview-item" title="{{ $attachment->file_name }}">
                                    <a href="{{ route('download_attachment', $attachment->id) }}" target="_blank" class="d-block">
                                        <div class="thumb">
                                            <img src="{{ static_asset($attachment->file_name) }}" class="img-fit" alt="no.1marry">
                                        </div>
                                        <div class="body">
                                            <h6 class="d-flex">
