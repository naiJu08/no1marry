              ...
              <table class="table aiz-table mb-0">
                <thead>
                    <tr>
                        ...
                        <th data-breakpoints="md">{{translate('Buyed Member Name')}}</th>
                    ...
                    @foreach ($package_payments as $key => $package_payment)
                        <tr>
                            <td>{{ ($key+1) + ($package_payments->currentPage() - 1)*$package_payments->perPage() }}</td>
                                ...
                                <td>
                                    @if($package_payment->profile != null)
                                        {{ $package_payment->profile->first_name . ' ' . $package_payment->profile->last_name }}
                                    @else
                                        {{ translate('Unknown') }}
                                    @endif
                                </td>
                            ...
                            <td>
                                  <span class="badge badge-inline badge-success text-center">{{ ucwords($package_payment->status) }}</span>
                            </td>
                            <td>{{ $package_payment->payment_id }}</td>
